...
const PLATFORM_RESET_PATH = '/api/v1/platform/demo/reset';
...
const RESET_REQUEST_ID_PATH = hostedResetRequestIdPath();
...
function recordFailedResetRequestId(request, upstreamResponse, host) {
  if (
    !RESET_REQUEST_ID_PATH
    || host !== PLATFORM_HOST
    || request.method !== 'POST'
    || request.url !== PLATFORM_RESET_PATH
    || (upstreamResponse.statusCode || 0) < 500
  ) return;

  const requestId = responseRequestId(upstreamResponse.headers);
  const evidence = UUID_PATTERN.test(requestId || '') ? requestId : 'invalid';
  writeFileSync(RESET_REQUEST_ID_PATH, `${evidence}\n`, {
    encoding: 'utf8',
    flag: 'wx',
    mode: 0o600,
  });
...
function proxyRequest(request, response, host, target) {
  const upstream = httpsRequest({
    ...
    recordFailedResetRequestId(request, upstreamResponse, host);
    response.writeHead(
