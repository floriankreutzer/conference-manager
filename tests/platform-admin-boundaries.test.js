...
test('Platform Admin runtime graphs may share capability code without selecting each other', () => {
  const violations = platformAdminBoundaryViolations({
    ...
    'src/platform-admin/demo/demo-store.js': 'export const key = globalThis.localStorage;',
  });
...
test('Demo network authority and Production browser authority are rejected', () => {
  const violations = platformAdminBoundaryViolations({
  ...
  assert.ok(violations.some((item) => item.includes('Demo modules must not create network')));
