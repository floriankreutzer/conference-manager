(function(){
 ...
 function bindOverlays(){for(const id of ['requestDetailOverlay','guestInfoOverlay','floorplanModalOverlay','reasonOverlay','cancelOverlayV24']){const el=document.getElementById(id);if(!el||el.__cmI18nOverlay)continue;const o=new MutationObserver(()=>schedule(el));o.observe(el,{childList:true,subtree:true});el.__cmI18nOverlay=o;apply(el)}}
 function observe(){for(const el of [document.querySelector('main.main'),document.querySelector('.sidebar')])if(el&&!el.__cmI18n){const o=new MutationObserver(()=>schedule(el));o.observe(el,{childList:true,subtree:true});el.__cmI18n=o}bindOverlays();if(document.body&&!document.body.__cmI18n){const o=new MutationObserver(m=>{if(m.some(x=>x.addedNodes.length)){bindOverlays();schedule(document.body)}});o.observe(document.body,{childList:true,subtree:false});document.body.__cmI18n=o}}
