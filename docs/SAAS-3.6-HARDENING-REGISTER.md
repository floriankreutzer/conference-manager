...
| H-032 | High / P1 security-evidence integrity | Exact DAST raw evidence | security-relevant evidence | PR #189 review found that upstream Baseline generation set `maxAlertsPerRule: 10`. ZAP applies that ceiling to the base passive scanner plugin, so later URLs can be suppressed before the raw report reaches the exact verifier; Platform plugin `90005` reached that boundary in live evidence. | The workflow now generates and validates its exact seven-job Automation Framework plan in the repository with `maxAlertsPerRule: 0`, runs the official ZAP image by its resolved local image ID and uploads the unfiltered reports. The verifier rejects any capped or structurally changed plan before accepting evidence. The container mounts only a dedicated plan/report directory rather than the repository tree. | **FIXED ON BRANCH — pending exact-head three-target DAST, fresh security review and merge** |
