...
| H-032 | High / P1 security-evidence integrity | Exact DAST raw evidence | security-relevant evidence | PR #189 review found that upstream Baseline generation set `maxAlertsPerRule: 10`. ZAP applies that ceiling to the base passive scanner plugin, so later URLs can be suppressed before the raw report reaches the exact verifier; Platform plugin `90005` reached that boundary in live evidence. Later reviews found that a path target did not itself prove a subtree restriction, an existing evidence-path symlink could defeat the intended narrow bind mount, and the direct Automation Framework start had dropped the wrapper's `-a` alpha passive rules. | The workflow now generates and validates its exact seven-job Automation Framework plan in the repository with `maxAlertsPerRule: 0`, explicit `90004`/`90005` thresholds, an exact target-subtree context include and an explicitly context-bound spider. In the same container home it installs beta and alpha passive add-ons, records the installed manifest, and refuses evidence unless both expected rule sets are proven. Before the container starts, the workflow removes the exact evidence path without following links, recreates a real directory and verifies its resolved location; the container mounts only that directory rather than the repository tree. | **FIXED ON BRANCH — pending exact-head three-target DAST, fresh security review and merge** |
