...
The cross-repository topology and consolidation baseline was accepted for **SaaS 3.5 — Architecture Consolidation & Shared Demo Runtime**. The customer role and configuration-ownership rows are amended by the approved **SaaS 3.6 — Role Model, Security Regression & Documentation Baseline**.

- SaaS 3.5 topology roadmap: [#149](https://github.com/floriankreutzer/conference-manager/issues/149)
- SaaS 3.5 architecture audit: [#150](https://github.com/floriankreutzer/conference-manager/issues/150)
- SaaS 3.6 role/ownership roadmap: [#164](https://github.com/floriankreutzer/conference-manager/issues/164)
- SaaS 3.6 ownership delivery: [#165](https://github.com/floriankreutzer/conference-manager/issues/165) and [#166](https://github.com/floriankreutzer/conference-manager/issues/166)
- Current approved roadmap baseline: version 11, 2026-09-01
...
The SaaS 3.5 topology, repository and shared-Demo persistence decisions remain accepted. `docs/ROLE-MODEL.md` and the matrix below supersede only earlier statements that assigned all Locations/Rooms/Catalogue business administration to Tenant Admin. Historical SaaS 3.5 evidence remains historical; this document does not claim that unmerged SaaS 3.6 work, a live deployment or milestone acceptance has completed.
...
9. Every active customer User receives the implicit Employee baseline. `conference_manager` and `tenant_admin` are independent elevated roles; a dual-role Principal receives their exact permission union and neither role inherits the other.
10. Configuration ownership is field-specific. Conference Manager owns Room business fields, the Tenant Catalogue and authoritative Room prices. Tenant Admin owns Site configuration, Room stable identity/technical Site assignment, provider integration/mapping administration, organization, policy, cost allocation, Users/roles, integrations and Tenant audit. Provider identity/resource references remain server/provider-controlled, and cost/cost-center reporting remains unassigned in SaaS 3.6.
11. The browser submits mutation intent only. The backend reclassifies the requested field changes against the persisted same-Tenant snapshot; a mutation spanning both ownership classes requires both elevated roles.
12. Provider-backed Rooms and referenced historical business records are deactivated, archived or cancelled through their approved lifecycle. Browser-facing business administration does not create provider identity or physically delete protected history.
...
| Customer User, Principal, role and permission authority | `conference-manager-api/src/identity/`, `src/authorization/`, and bounded Tenant User application services | Customer session/API adapters; frontend `src/platform/production-session.js` and Tenant Admin Users section | The server adds the implicit Employee baseline and derives the exact permission union for zero, one or both independent elevated roles. Browser roles, User IDs, Tenant IDs and permissions are presentation input only. Tenant roles never create Platform authority. |
...
| Request, booking workflow, history and confirmed-change semantics | `conference-manager-api/src/domain/request*.js`, booking-change domain, and bounded Request/booking application services | Employee and Conference Manager frontend applications; customer HTTP/repository adapters; provider adapters behind the calendar port | Employee scope is own Requests; Conference Manager scope is the authenticated Tenant. The server owns workflow, ownership, version, price/configuration snapshots, availability and final transitions. An eligible Conference Manager self-decision persists distinct server-derived initiator/decider audit evidence. Browser validation is advisory. |
| Site configuration, Room stable identity, technical Site assignment and provider mapping | `conference-manager-api/src/domain/tenant-locations.js`, Tenant Location application/repository owner and bounded provider-mapping services | Tenant Admin Locations and Microsoft 365 sections; Employee/Manager read projections; Microsoft room mapping adapter | Tenant Admin may administer only the technical/Site/provider fields allowed by the canonical contract. Provider identity/resource references remain server/provider-owned, provider-backed Rooms are not manually created or physically deleted, and technical mutation does not overwrite Conference Manager-owned business fields. |
| Room business data | `conference-manager-api/src/domain/tenant-locations.js` and the Tenant Location application/repository owner | Conference Manager business-settings presentation; Employee/Manager read projections | Conference Manager owns `name`, `capacity`, `active`, `floor`, `equipment`, `accessibility`, `serviceIds`, `cateringPackageIds`, `floorplanAssetId` and `mediaAssetIds`. The API compares the complete replacement with the persisted same-Tenant snapshot, rejects technical-field changes without Tenant Admin authority, and requires both roles for a mixed mutation. |
| Service, equipment and catering Catalogue plus authoritative Room prices | `conference-manager-api/src/domain/tenant-catalogue.js` and the bounded Catalogue application/repository owner | Conference Manager business-settings presentation; Employee/Manager read projections | Catalogue mutation requires Conference Manager authority and remains Tenant-scoped, revisioned and audit-atomic. Tenant Admin does not inherit Catalogue writes. There is no generic settings document or second browser-owned write implementation; SaaS 4 provider work adapts this owner rather than replacing it. |
...
| Conference Manager presentation and use-case intent | `conference-manager/src/manager/`, exposed only through `src/manager/index.js` | Customer composition root injects server-backed Request, Location and Catalogue ports | Manager owns presentation and mutation intent for Tenant-wide operational Requests, Room business fields, Catalogue and Room prices. It does not import Employee/Tenant Admin internals, establish server authority, or mutate technical provider/Site fields. |
| Tenant Admin presentation and section orchestration | `conference-manager/src/tenant-admin/`, exposed only through `src/tenant-admin/index.js` | Section adapters are injected by the customer composition root | Tenant Admin composes organization, technical Locations/provider mapping, booking policy, cost allocation, Users/roles, Microsoft integration, capability/readiness and audit sections. It has no Catalogue section and does not inherit Conference Manager business authority. Sections do not import one another or Platform/Employee/Manager internals; visibility is not authorization. |
...
| Shared server-backed Demo runtime | Demo-only backend composition, seed/reset orchestration and deterministic identity/provider adapters defined by ADR-010 | Customer Demo and Platform Demo browsers call their separate Demo API/session boundaries | One isolated PostgreSQL data model, not two synchronized fixture documents. Customer personas use the same Employee baseline, independent elevated roles and derived dual-role union as Production. Browser storage may hold non-authoritative UX preferences only. |
...
| SaaS 3.5 consolidated a legacy Conference Manager master-data path under the then-current SaaS 2 assumption that Tenant Admin owned Organization, Locations/Rooms, Catalogue, Booking Policy and Cost Allocation. | SaaS 3.6 explicitly supersedes that role allocation without replacing the bounded backend aggregates: Conference Manager owns Room business fields, Catalogue and Room prices; Tenant Admin owns the technical/provider Room boundary plus organization, policy and cost allocation. The API remains authoritative, and mixed Room changes require both roles. | #165 / #166 authorization, field-classification, cross-Tenant and capability-boundary tests |
...
| Tenant Admin in-memory Demo settings/User/Microsoft/audit/capability adapters and fixtures | Server-backed Customer Demo APIs using canonical Tenant settings, User, integration and audit application owners. The current role projection exposes technical/Tenant administration to Tenant Admin and Room-business/Catalogue administration to Conference Manager rather than restoring the former all-settings Tenant Admin assumption. |
...
Any new material responsibility must update this matrix in the same reviewed change that introduces the owner. A change that creates a second active owner, collapses the independent Conference Manager/Tenant Admin roles, changes the Room field classification without matching server authorization and negative tests, retains a bridge past its removal trigger, or adds an undocumented exception fails architecture review. File size, directory symmetry or repository count alone is never sufficient reason to move ownership.
