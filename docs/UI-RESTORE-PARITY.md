...
| `EMP-03` | Room selection | Visual Room cards, capacity/price/equipment, unavailable reasons, preview and floorplan dialog | Policy-filtered `roomEditorOptions`, server availability check and authoritative Site time zone | `RESTORE_REQUIRED`: active UI is a bare Room select | Card selection, capacity/no-room recovery, conflict, preview/floorplan, keyboard and safe asset tests |
| `EMP-04` | Services and Equipment | Selectable cards with applicability, visible price basis and retained selection | Server Catalogue projection filtered by selected Room/Site and booking policy | `RETAIN_AND_ENHANCE`: checkbox controls exist but approved card hierarchy and guidance are missing | Applicability, selection persistence, price-basis, empty/error and DE/EN tests |
...
| `EMP-14` | Guest Information | Rich guest dialog with schedule, route/arrival, building, accessibility/contact and safe Wi-Fi policy | `loadRequestRoomContext` supplies current/historical Room/Site presentation; confirmed Requests only | `RETAIN_AND_ENHANCE`: basic detached print data exists, rich in-app guest view is missing | Confirmed-only visibility, minimized fields, safe HTTPS route, focus return and missing-data tests |
| `EMP-15` | Print | Styled detached guest/welcome document with meaningful print action and no opener relationship | Server Request plus minimized Room/Site context; no internal identifiers, secrets or tokens | `RETAIN_AND_ENHANCE`: a minimal print window exists; approved layout/route context is missing | Strict CSP, `opener === null`, no token/internal leakage, print CSS and popup-blocked behavior |
...
| `MGR-06` | Confirmed changes and self-approval | Propose/decide confirmed changes; self-approval retains distinct initiator/decider evidence | `proposeBookingChange`, `loadBookingChange`, `decideBookingChange`; server attribution authoritative | `RETAIN_CURRENT` for contracts; surface audit meaning in restored details/timeline | Initiator/decider, own proposal, conflict/rejection and cross-Tenant tests |
...
| `MGR-08` | Timeline/history | Timeline visible from operational card/review context | `loadRequestHistory` server projection | `RETAIN_AND_ENHANCE`: history is dialog-only and disconnected from cockpit hierarchy | Ordering, localization, audit actor semantics, empty/error and focus tests |
