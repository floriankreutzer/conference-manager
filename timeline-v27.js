(function(){
 ...
 function captureConfirmed(){document.addEventListener('click',e=>{const b=e.target.closest('[data-manager-action="confirm"]');if(!b)return;const id=b.dataset.requestId;setTimeout(()=>{const rs=get(),r=rs.find(x=>x.id===id);if(!r||r.status!=='Confirmed')return;const exists=(r.statusHistory||[]).some(x=>x.status==='Confirmed');if(!exists){window.cmWorkflow?.appendHistory?.(r,'Confirmed','',new Date().toISOString());save(rs)}render();window.dispatchEvent(new CustomEvent('conference-request-updated'))},100)},true)}
 function init(){render();captureConfirmed();const list=q('#requestList');if(list&&!obs){obs=new MutationObserver(render);obs.observe(list,{childList:true})}window.addEventListener('conference-request-updated',()=>setTimeout(render,0));document.documentElement.dataset.timelineBuild='2026.08.22.28'}
