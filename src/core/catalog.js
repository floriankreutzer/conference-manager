...
function migrateLocalized(value, defaultValue = null) {
  if (value && typeof value === 'object' && 'de' in value && 'en' in value) return value;
  const text = String(value ?? '');
  if (defaultValue && text === String(defaultValue.de ?? '')) return structuredClone(defaultValue);
...
function migrateCatalog(catalog) {
  ...
  next.rooms = (next.rooms || []).map((room) => {
    const fallback = DEFAULT_CATALOG.rooms.find((entry) => entry.id === room.id);
    return {
      ...room,
      name: migrateLocalized(room.name, fallback?.name),
      equipment: migrateLocalized(room.equipment, fallback?.equipment),
      floor: migrateLocalized(room.floor || '', fallback?.floor),
    };
  });
  next.services = (next.services || []).map((service) => {
    const fallback = DEFAULT_CATALOG.services.find((entry) => entry.id === service.id);
    return {
      ...service,
      name: migrateLocalized(service.name, fallback?.name),
      description: migrateLocalized(service.description ?? service.desc ?? '', fallback?.description),
    };
  });
  next.cateringPackages = (next.cateringPackages || []).map((pack) => {
    const fallback = DEFAULT_CATALOG.cateringPackages.find((entry) => entry.id === pack.id);
    return {
      ...pack,
      name: migrateLocalized(pack.name, fallback?.name),
      description: migrateLocalized(pack.description || '', fallback?.description),
      variants: (pack.variants || []).map((variant) => {
        const fallbackVariant = fallback?.variants.find((entry) => entry.tier === variant.tier);
        return {
          ...variant,
          description: migrateLocalized(variant.description ?? variant.desc ?? '', fallbackVariant?.description),
        };
      }),
    };
  });
  next.cateringItems = (next.cateringItems || []).map((item) => {
    const fallback = DEFAULT_CATALOG.cateringItems.find((entry) => entry.id === item.id);
    return {
      ...item,
      name: migrateLocalized(item.name, fallback?.name),
      unit: migrateLocalized(item.unit, fallback?.unit),
    };
  });
  return next;
}

const LOCALIZED_SITE_FIELDS = Object.freeze([
  'publicTransport', 'carArrival', 'parking', 'reception', 'building',
  'visitorNotes', 'accessibility', 'wifiInstructions',
]);

function migrateSiteInfo(siteInfo) {
  const next = structuredClone(siteInfo || DEFAULT_SITE_INFO);
  for (const [location, site] of Object.entries(next)) {
    const fallback = DEFAULT_SITE_INFO[location];
    if (!fallback) continue;
    for (const field of LOCALIZED_SITE_FIELDS) {
      site[field] = migrateLocalized(site[field] ?? '', fallback[field]);
    }
  }
  return next;
}
...
export function loadSiteInfo() {
  ...
  const migrated = migrateSiteInfo(current || DEFAULT_SITE_INFO);
  writeJson(KEYS.siteInfo, migrated);
  return migrated;
}
