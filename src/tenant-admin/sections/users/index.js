...
export function createUsersSection({ context, adapter = null } = {}) {
  ...
  function userCard(user, rerender) {
    ...
    card.append(header, baseline, roles, message, el('div', { className: 'button-row tenant-user-actions' }, [save]));
    return card;
  }
  ...
  async function render({ root, isCurrent, rerender }) {
    ...
    try {
      ...
      users.forEach((user) => list.appendChild(userCard(user, rerender)));
